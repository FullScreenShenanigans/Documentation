...
The coordinate system in which these Things are placed has an origin at the top left corner (0,0). Things are also scaled from these raw measurements to actual in-game measurements using a predefined `unitsize`. A `unitsize` of 1 means no scaling, while a `unitsize` of 2 means in-game measurements are doubled.
...
var thing = FSP.GroupHolder.getGroup("Solid")[0];
var otherThing = FSP.GroupHolder.getGroup("Solid")[1];
```
...
FSP.shiftVert(thing, 1);

// Shifts a Thing horizontally by 1. 
FSP.shiftHoriz(thing, 1);
```
...
FSP.setMidObj(thing, otherThing);

// Aligns just the horizontal midpoint. 
FSP.setMidXObj(thing, otherThing);

// Aligns just the vertical midpoint. 
FSP.setMidYObj(thing, otherThing);
```
...
FSP.setOpacity(thing, 1);

// Sets a Thing's opacity to 0 (transparent). 
FSP.setOpacity(thing, 0);
```
...
FSP.addThing("CastleFireball", 1, 2);

// Adds a Thing to the right of another Thing.
FSP.addThing("CastleFireball", otherThing.right, otherThing.top);

// Adds a Thing below another Thing.
FSP.addThing("CastleFireball", otherThing.left, otherThing.bottom);
```
...
FSP.addEvent(
    (thing: IThing): void => FSP.shiftHoriz(thing, 2),
    100);
```
